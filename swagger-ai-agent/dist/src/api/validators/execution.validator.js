...
exports.validatePlanExecutionRequest = validatePlanExecutionRequest;
exports.validateExecuteRunRequest = validateExecuteRunRequest;
exports.validateRetryFailedRequest = validateRetryFailedRequest;
function isObject(value) {
    return typeof value === 'object' && value !== null;
}
function validatePlanExecutionRequest(body) {
    if (!isObject(body)) {
...
function validateExecuteRunRequest(body) {
    if (!isObject(body)) {
        return [{ field: 'body', message: 'Body must be a JSON object' }];
    }
    const record = body;
    const hasRunId = typeof record.runId === 'string' && record.runId.trim() !== '';
    const hasPlanParams = typeof record.specId === 'string' &&
        record.specId.trim() !== '' &&
    ...
    return validatePlanExecutionRequest(body);
}
function validateRetryFailedRequest(body) {
    if (!isObject(body)) {
        return [{ field: 'body', message: 'Body must be a JSON object' }];
    }
...
//# sourceMappingURL=execution.validator.js.map